...
int builtin_command(struct cmdline_tokens *t);
pid_t fgpid(struct job_t *job_list);
struct job_t *getjobpid(struct job_t *job_list, pid_t pid);
struct job_t *getjobjid(struct job_t *job_list, int jid);
int pid2jid(pid_t pid);
void listjobs(struct job_t *job_list, int output_fd);
void bg_fg(struct cmdline_tokens *t);
void usage(void);
...
{
    int bg;
    int rin,rout; /* The job run in bg or fg.  Redirect in & out. */
    pid_t pid;
    struct cmdline_tokens t;
    /* Parse command line */
    bg = parseline(cmdline, &t);
    sigset_t mask;
    sigset_t mask0;

    if (bg == -1) return;               /* parsing error */
    if (t.argv[0] == NULL)  return;   /* ignore empty lines */
    ...
    if(!builtin_command(&t))
    {
		...
		if(sigaddset(&mask, SIGCHLD) != 0 && sigaddset(&mask, SIGINT) != 0 && sigaddset(&mask, SIGTSTP)!= 0)
		{
		...
		else if(pid == 0)
		{
            ...
            if(t.outfile!=NULL)
            {   if((rout=(open(t.outfile,O_RDWR)))!=-1)
                {
                    //f_out=fopen(t.outfile,"w");
                 dup2(rout, 1);
               /* if(f_out!=NULL)
                fclose(f_out);*/
            ...
            else if(t.infile!=NULL)
            {
                if((rin=(open(t.infile,O_RDONLY)))!=-1)
                {     //{    f_in=fopen(t.infile,"r");
			...
			{
				printf("%s: Command can not be found\n", t.argv[0]);
				exit(1);
			}
		...
		else
		{
			...
			if (!bg)
			{
               ...
               {
                    sigemptyset(&mask0); //Empty the mask2 to be used as a parameter in sigsuspend
		            sigsuspend(&mask0);
			   }
	   ...
	   /* else builtin_command(&t);

        if (t.infile != NULL) dup2(dup(0), 0);
        if (t.outfile != NULL) dup2(dup(1), 1);*/
...
int builtin_command(struct cmdline_tokens *t)
{
    ...
    switch(t->builtins)
   {
     ...
     case BUILTIN_JOBS:
         if (t->outfile != NULL)
       {
           rout =(open(t->outfile, O_RDWR));
           listjobs(job_list, rout);
...
/*The function deals with the bg and fg situations.*/
void bg_fg(struct cmdline_tokens *t)
{
        int jid;
        struct job_t *job;
...
sigint_handler(int sig)
{
	...
	if (pid != 0)
	{
		...
		/*if (sig < 0)
		{
			printf("The job [%d] (%d) is terminated by signal %d\n", jid, pid, (-sig));
			//deletejob(job_list, pid);
		}*/
	}
    return;
...
sigtstp_handler(int sig)
{
	...
	if (pid != 0)
	{
		//printf("The job [%d] (%d) is stopped by signal %d\n", jid, pid, sig);
